...
struct scalar* build_top(struct data elem, struct scalar *add_ver)
{
	...
	if(flag)
		tmp->right = build_node(elem);
	else
		tmp->left = build_node(elem);
	return tmp;
}
...
unsigned char length_prefix(struct scalar *tmp, struct data elem, char *flag)
{
	unsigned char number_node = tmp->number, car, number_search_2, shift;
	unsigned int number_search = elem.number, i = tmp->length;
...
void break_top(struct scalar **tmp, struct data elem, char length, char bit)
{
...
void add_in_scalar_tree(struct scalar **head, struct data elem)
{
	char bit;
	if(!(*head)){
	...
	if(length == tmp->length){
		if(32 - data.length == elem.mask)
			...
			add_in_scalar_tree(&((*head)->right), elem);
		else
			add_in_scalar_tree(&((*head)->left), elem);
	} else {
...
void insert_in_scalar_tree(struct scalar **head, struct top32 elem)
{
	if(!(*head)){
...
char search_in_scalar_tree(struct scalar **head, struct data info, char *key)
{
	struct scalar *tmp;
	char bit, flag = 0;
...
{
	struct node *tmp = *head, *parent = *head, *grand = *head;
...
void delete_from_scalar_tree(struct scalar **head, struct top32 elem)
{
	struct data info = {elem.number, elem.key, elem.mask, elem.mask};
	if(data.number & 2147483648)
